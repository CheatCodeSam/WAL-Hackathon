...
export default function ChannelPodcasts({ channelId }: Props) {
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow">
      ...
      {error && (
        <div className="rounded-md bg-red-50 p-3 text-red-800">Failed to load podcasts: {error.message}</div>
      )}
      ...
      <ul className="space-y-4">
        {podcasts?.map((p: any, idx: number) => {
          const created = p.created_at ? new Date(Number(p.created_at)).toLocaleDateString() : p.created_at || 'Unknown';
            ...
            <li key={idx} className="overflow-hidden rounded-md border p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-xs text-gray-500">{created} · {p.file_type || 'unknown'}</div>
                </div>
