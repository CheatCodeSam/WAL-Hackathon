...
export function useCreatorDashboard() {
  ...
  const [errorState, setErrorState] = useState<'connect_wallet' | 'create_channel' | null>()
  const [activeTab, setActiveTab] = useState<'channel' | 'podcasts' | 'metrics'>('channel');
  ...
  const saveChannel = async () => {
    try {
      ...
      const tx = ServiceChannel.init().create({
