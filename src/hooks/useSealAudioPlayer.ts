		...
		try {
			...
			tx.moveCall({
				...
				arguments: [
					tx.pure.vector(
						"u8",
						Array.from(new TextEncoder().encode(options.nonce)),
					),
			...
			try {
				...
				if (devInspectResult.effects.status.status !== "success") {
					const error =
						devInspectResult.effects.status.error || "Unknown error";
				...
				const errorMsg =
					devInspectError instanceof Error
						? devInspectError.message
						: String(devInspectError);

				// Parse common error messages
				if (errorMsg.includes("ESubscriptionExpired")) {
					throw new Error(
						"Your subscription has expired. Please renew to access this content.",
					);
					...
					throw new Error(
						"Invalid nonce - the encryption key doesn't match this podcast.",
					);
				} else {
