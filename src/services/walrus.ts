...
type WalrusStoreResponse = WalrusNewlyCreatedResponse | WalrusAlreadyCertifiedResponse;
...
export async function uploadToWalrus(
  ...
  try {
    ...
    return {
      ...
      url: accessUrl,
      size: file.size,
      certifiedEpoch: "newlyCreated" in data ? data.newlyCreated.blobObject.certifiedEpoch ?? undefined : undefined,
    };
  } catch (error) {
...
export async function uploadQuilt(
  ...
  try {
    ...
    const quiltId = "newlyCreated" in data.blobStoreResult 
      ? data.blobStoreResult.newlyCreated.blobObject.blobId
      : data.blobStoreResult.alreadyCertified.blobId;

    const patches = data.storedQuiltBlobs.map((blob: { identifier: string; quiltPatchId: string }) => ({
